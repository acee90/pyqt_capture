...
from PyQt6.QtWidgets import QDialog, QFrame, QApplication, \
                            QVBoxLayout, QLabel, QWidget, QToolButton, QHBoxLayout, \
                            QSizePolicy
...
class TitleBar(QDialog):
    ...
    def mousePressEvent(self,event):
        if event.button() == Qt.MouseButton.LeftButton:
            box.moving = True; box.offset = event.position()

    def mouseMoveEvent(self,event):
        if box.moving: 
            box.move((event.globalPosition() - box.offset).toPoint())
