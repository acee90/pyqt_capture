...
from PyQt6.QtWidgets import QApplication, QMainWindow, QDialog, \
                            QWidget, QSizeGrip, QMessageBox
from PyQt6 import QtCore, QtGui

from multiprocessing import Process, Queue
import multiprocessing as mp
import tesseract_proc
...
class TesseractThread(QtCore.QThread):
...
class MyApp(QMainWindow):
    ...
    def __init__(self, input_q, output_q):
        ...
        self.consumer = TesseractThread(input_q, output_q)
        self.consumer.sig_done.connect(self.onTesseractDone)
    ...
    def translate(self, ocr_string: str):
        if (len(ocr_string) == 0):
            return
        ...
        if response.status_code == 200:
            ...
            errorMsg: str = response.json()['errorMessage']
            self.statusBar().showMessage(errorMsg)
            if (response.json()['errorCode'] != '010'):
                QMessageBox.about(self, 'Error', errorMsg)
    ...
    def onTesseractDone(self, ret):
        print('onTesseractDone')
    
        self.pictureButton.setDisabled(False)
        self.textEdit_ocr.setPlainText(ret)
        self.statusBar().showMessage('translate...')
        self.translate(ret)

    def startTesseract(self, img):
        self.pictureButton.setDisabled(True)
        self.statusBar().showMessage('ocr....')
        self.consumer.tesseract(img)
...
if __name__ == '__main__':
    ...
    # producer process
    p = Process(name="producer", target=tesseract_proc.producer, args=(input_q, output_q), daemon=True)
    p.start()
